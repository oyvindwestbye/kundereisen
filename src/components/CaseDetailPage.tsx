...
const mockTimeline = [
  ...
  { status: 'Dokumentasjon sendt', date: '14. jan 2025', completed: true, link: 'https://ocab.no/vi-kommer-snart-pa-besok/' },
  { status: 'Godkjenning fra forsikring', date: '15. jan 2025', completed: true, link: 'https://ocab.no/vi-har-fatt-bekreftelse-fra-forsikringsselskapet-ditt/' },
...
const mockDocuments = [
  ...
  { id: 3, name: 'Godkjenning_forsikring.pdf', type: 'application/pdf', size: 120, uploadedBy: 'Selskap A', date: '15. jan 2025' },
...
const mockActivityLog = [
  { action: 'Dokument lastet opp', user: 'Ola Svendsen', details: 'Godkjenning_forsikring.pdf', timestamp: '15. jan 2025, 09:20' },
  { action: 'Status oppdatert', user: 'System', details: 'Fra "Venter godkjenning" til "Reparasjon starter"', timestamp: '15. jan 2025, 09:15' },
  { action: 'Melding sendt', user: 'Selskap A', details: 'Godkjenning bekreftet', timestamp: '15. jan 2025, 09:15' },
...
export function CaseDetailPage({ caseId, onBack }: CaseDetailPageProps) {
  const [activeTab, setActiveTab] = useState<'dokumenter' | 'chat'>('chat');
  const [files, setFiles] = useState<File[]>([]);
  const [showDeleteModal, setShowDeleteModal] = useState(false);
  const [itemToDelete, setItemToDelete] = useState<{ type: 'document', id: number } | null>(null);
    ...
    <div className="min-h-screen bg-[#fafafa]">
      ...
      <div className="bg-white border-b border-[#dde1e6]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
            <div className="flex-1">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    <p className="text-xs text-[#697077]">Kunde</p>
                    <p className="text-sm">{mockCaseData.customer.name}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
                  <Building2 size={16} className="text-[#697077]" />
                  <div>
                    <p className="text-xs text-[#697077]">Forsikring</p>
                    <p className="text-sm">{mockCaseData.insurance}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2">
            ...
            {/* Contact Info Card */}
            <div className="bg-[#f4f4f4] rounded-lg p-4 min-w-[280px]">
              <h6 className="mb-3">Kontaktinformasjon</h6>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-1">
            <div className="bg-white rounded-lg border border-[#dde1e6] p-6 sticky top-8">
          ...
          <div className="lg:col-span-2">
            {/* Tabs */}
            <div className="bg-white rounded-t-lg border border-b-0 border-[#dde1e6] flex">
              <button
                onClick={() => setActiveTab('chat')}
                className={`flex-1 px-6 py-3 border-b-2 transition-colors ${
                  activeTab === 'chat'
                    ? 'border-[#016cb7] text-[#016cb7]'
                    : 'border-transparent text-[#697077] hover:text-[#21272a]'
                }`}
              >
                Chat
              </button>
            ...
            {/* Tab Content */}
            <div className="bg-white rounded-b-lg border border-[#dde1e6] p-6 min-h-[600px]">
              {/* Documents Tab */}
              {activeTab === 'dokumenter' && (
                <div className="space-y-4">
                  <div className="flex items-center justify-between mb-4">
                    <h4>Dokumenter ({mockDocuments.length})</h4>
                  </div>
                  ...
                  <div className="space-y-2 mt-6">
                    {mockDocuments.map((doc) => (
                      <div 
                        key={doc.id}
                        className="flex items-center gap-3 p-4 bg-[#f4f4f4] rounded-lg hover:bg-[#e7efff] transition-colors group"
                      >
      ...
      >
        <p>
          ...
          {itemToDelete?.type === 'document' ? 'dokumentet' : 'meldingen'}?
